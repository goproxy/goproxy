...
func (g *Goproxy) ServeHTTP(rw http.ResponseWriter, req *http.Request) {
	...
	if strings.HasPrefix(name, "sumdb/") {
		...
		if err := httpGet(
			...
			g.logErrorf(
				"proxy checksum database request: %s",
	...
	if err != nil {
		g.logErrorf("create temporary directory: %v", err)
